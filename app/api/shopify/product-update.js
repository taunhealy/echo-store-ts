...
export default async function send(req, res) {
  ...
  const shopifyProduct = await axios({
    url: `https://${process.env.NEXT_PUBLIC_SHOPIFY_STORE_ID}.myshopify.com/admin/api/2024-01/products/${id}/metafields.json`,
    method: 'GET',
  ...
  if (previousSync) {
    ...
    if (jsondiffpatch.diff(JSON.parse(previousSync.value), productCompare)) {
      ...
      axios({
        url: `https://${process.env.NEXT_PUBLIC_SHOPIFY_STORE_ID}.myshopify.com/admin/api/2024-01/products/${id}/metafields/${previousSync.id}.json`,
        method: 'PUT',
        headers: shopifyConfig,
        data: {
          metafield: {
